...
class TimeHashSlabNonContig:
    ...
    - step0: strided along axis 0, contiguous along the innermost axis
      (hashed as one EVP_DigestUpdate per row, without any copy)
    - transpose: transposed slab (chunk-sized scratch buffer path)
    - step_inner: strided along the innermost axis (chunk-sized scratch buffer
      path)
    - broadcast: read-only broadcasted slab, e.g. the full slab of a
      StagedChangesArray (chunk-sized scratch buffer path)
    """

    params = [["step0", "transpose", "step_inner", "broadcast"], CHUNK_SIZES]
