...
def commit_staged_changes(
    ...
    #    commit() appends a new base slab only if at least one staged chunk survives
    #    deduplication; if every change deduplicated against an existing raw_data (or
    #    full) chunk, no new slab is created and n_base_slabs is left unchanged.
    n_base_before = sc.n_base_slabs
    sc.commit(empty=_empty)
    appended = sc.n_base_slabs > n_base_before

    if appended:
        # 3. Shift the new chunks' offsets from view-relative to absolute raw_data
        #    offsets, so each new non-full chunk's slab_offset is its position in
        #    raw_data. The new base slab is the one commit() just appended, at the end.
        ...
        # Nothing was written to raw_data: every surviving chunk already references an
        # existing base slab (absolute offsets) or the full slab. Use a sentinel slab
        # index that no chunk can match, so no offsets are shifted and no hashes are
        # (re-)recorded below.
        new_idx = -1
        new_hash_table = None
    ...
    # is written exactly once. Skipped entirely when no new slab was appended.
    if new_chunk_slices:
        assert new_hash_table is not None
        with Hashtable(f, name) as hashtable:
    ...
    if appended:
        # Fold the new base slab into the single raw_data slab at index 1. (When nothing
        # was appended, surviving chunks already point at slab 0 or 1, so no remap.)
